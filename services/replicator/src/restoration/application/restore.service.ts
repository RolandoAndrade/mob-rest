...
export class RestoreService {
    constructor(
        ...
        this.serverManager.addEventFromServer(
            RestorationMessages.RETRIEVE_OBJECTS,
            this.onRetrieveObjects.bind(this),
        );
    }

    async onRetrieveObjects() {
        this.loggerService.log(
            'onRetrieveObjects: sending replication file',
            'RestoreService',
        );
        const file = fs
            .readFileSync(this.configManager.get('repository'))
            .toString('base64');
        this.serverManager.sendMessageToServers(
            RestorationMessages.RECEIVED_OBJECTS,
            file,
        );
    }
}
