...
export class RestoreService {
    ...
    async onRestoreRequest() {
        this.loggerService.log(
            'onRestoreRequest: request of restoration in progress',
            'RestoreService',
        );
    ...
    async onReceivedObject(client: Socket, base64StringData: string) {
        this.loggerService.log(
            'onReceivedObject: received objects',
            'RestoreService',
        );
        this.restoredObjects.push(base64StringData);
        if (
            this.restoredObjects.length ===
            (this.server.engine as any).clientsCount
        ) {
            this.loggerService.log(
                'onReceivedObject: received all objects, comparing them',
                'RestoreService',
            );
            const areAllReceivedElementsEqual = this.restoredObjects.every(
                (element) => element === base64StringData,
            );
            if (areAllReceivedElementsEqual) {
                this.loggerService.debug(
                    'onReceivedObject: all received elements are equal, restoring the information',
                    'RestoreService',
                );
                fs.writeFileSync(
                ...
                this.loggerService.warn(
                    "onReceivedObject: there are objects that don't equal, restoring the mode element",
                    'RestoreService',
                );
                fs.writeFileSync(
                    this.configManager.get('repository'),
