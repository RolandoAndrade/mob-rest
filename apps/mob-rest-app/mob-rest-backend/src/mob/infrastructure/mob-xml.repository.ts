...
export class MobXmlRepository implements MobRepository {
    constructor(private readonly configService: ConfigManager) {}
    ...
    private getLibrary(): Library {
        ...
        const xmlFile = fs
            .readFileSync(this.configService.get('repository'))
            .toString();
    ...
    public async deleteObject(query: FindQuery): Promise<boolean> {
        ...
        if (library && library.library && library.library.book) {
            ...
            library.library.book = library.library.book.filter(
                (book: ParsedBook) => {
    ...
    public async findObject(findQuery: FindQuery): Promise<Book[]> {
        ...
        if (library && library.library && library.library.book) {
            ...
            results = library.library.book
                .filter((book: ParsedBook) => {
                    return (
                        this.checkFilter(book, findQuery) ||
                        (!findQuery.name &&
                            !findQuery.surname &&
                            !findQuery.title)
                    );
    ...
    public async updateObject(
        ...
        if (library && library.library && library.library.book) {
            ...
            library.library.book = library.library.book.map(
                (book: ParsedBook) => {
                        ...
                        return {
                            ...
                            author: {
                                name: newBook.author
                                    ? newBook.author.name
                                    : book.author.name,
                                surname: newBook.author
                                    ? newBook.author.surname
                                    : book.author.surname,
                            },
